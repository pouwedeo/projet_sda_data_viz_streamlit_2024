...
# Scraping de l'url
text_read = scrape_kickstarter(url)
...
def corpus_delete_stopwords(corpus):
    ...
    corpus_no_stopword_splited = [' '.join([word for word in doc.split() if word not in stopwords_clean]) for doc in corpus_delete_caracter(corpus)]
