...
#Class to define projects success Factors graph
class GraphFactClass(Dashboard):
    ...
    def categorie_fact_project(self):
        try:
            success_rate_by_category = self.data.groupby("category")["state"].apply(
                lambda x: (x == "successful").mean() * 100
            ).reset_index(name="success_rate")

            # Ajouter une colonne pour le nombre total de projets par catégorie
            success_rate_by_category["total_projects"] = self.data.groupby("category")["state"].count().values
            ...
            fig_cat_fact = px.bar(top_categories, x='category', y='success_rate', hover_data='total_projects',
                                  hover_name='total_projects',
                                  title='Taux de succès par catégorie (Top catégories qui ont plus de projets)',
                                  labels={'category': 'Categories', 'success_rate': 'Taux de succès',
                                          'total_projects': 'Nombre de projets'}, text='success_rate')
    ...
    def categorie_fact_stat(self):
        try:
            ...
            # Ajouter une colonne pour le nombre total de projets par catégorie
            success_rate_by_category["total_projects"] = self.data.groupby("category")["name"].count().values
    ...
    def time_fact(self):
        try:
            self.data["duration_bins"] = pd.cut(self.data["duration"], bins=[0, 10, 20, 30, 40, 50, float('inf')],
                                           labels=["<10 jours", "10-20 jours", "20-30 jours", "30-40 jours",
                                                   "40-50 jours", ">50 jours"])

            # Calculer le taux de succès pour chaque intervalle
            success_rate_by_duration = self.data.groupby("duration_bins")["state"].apply(
            ...
            fig_time_fact = px.bar(success_rate_by_duration, x='duration_bins', y='success_rate', text='success_rate',
                                   ...
                                   labels={'duration_bins': 'Durée de la campagne',
                                           'success_rate': 'Taux de succès en %'}, barmode='group')
    ...
    def place_fact(self):
        try:
            success_rate_by_country = self.data.groupby("country")["state"].apply(
                lambda x: (x == "successful").mean() * 100).reset_index(name="success_rate")
    ...
    def amount_fact(self):
        try:
            self.data["goal_bins"] = pd.cut(self.data["goal"], bins=[0, 1000, 5000, 10000, 50000, 100000, float('inf')],
                                       labels=["<1k", "1k-5k", "5k-10k", "10k-50k", "50k-100k", ">100k"])

            # Calculer le taux de succès pour chaque intervalle
            success_rate_by_goal = self.data.groupby("goal_bins")["state"].apply(
                lambda x: (x == "successful").mean() * 100).reset_index(name="success_rate")

            fig_amount_fact = px.pie(success_rate_by_goal, names="goal_bins", values="success_rate",
                                     title="Taux de succès en fonction du montant demandé",
                                     color_discrete_sequence=px.colors.qualitative.Set3,
                                     labels={'goal_bins': 'Montant demandé (USD)',
                                             'success_rate': 'Taux de succès en %'})
